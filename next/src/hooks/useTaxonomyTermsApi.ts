...
interface UseTaxonomyTermsApiOptions {
  ...
  initialValue?: string | string[] // Начальное значение (ID термина или массив ID)
  searchQuery?: string // Поисковый запрос
  limit?: number // Ограничение количества результатов
  searchDebounce?: number // Задержка для поиска в миллисекундах
  enabled?: boolean // Включен ли запрос
}

interface UseTaxonomyTermsApiResult {
  terms: TaxonomyTerm[] // Доступные термины
  selectedTerm: TaxonomyTerm | null // Выбранный термин (для single select)
  selectedTerms: TaxonomyTerm[] // Выбранные термины (для multi select)
  isLoading: boolean
  isLoadingSelected: boolean // Загрузка выбранных терминов
  error: string | null
  searchTerms: (query: string) => void // Функция для поиска терминов
  fetchTerms: () => Promise<void> // Функция для принудительной загрузки терминов
  fetchTermById: (id: string) => Promise<TaxonomyTerm | null> // Функция для загрузки термина по ID
}
